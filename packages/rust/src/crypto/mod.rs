    ...
    match value {
        serde_json::Value::Object(map) => {
            let sorted: BTreeMap<_, _> = map.iter()
                .map(|(k, v)| (k.clone(), sort_keys(v))).collect();
...
pub fn construct_asset_id(pubkey_hex: &str, timestamp: i64, nonce_hex: &str) -> Result<String, L3rsError> {
...
pub fn construct_cid(asset_id: &str, state_hash: &str, compliance_hash: &str,
                     governance_hash: &str, timestamp: i64) -> Result<String, L3rsError> {
    let decode = |s: &str| hex::decode(s).map_err(|e| L3rsError::Crypto(e.to_string()));
    let id = decode(asset_id)?; let sh = decode(state_hash)?;
    let ch = decode(compliance_hash)?; let gh = decode(governance_hash)?;
    let ts = (timestamp as u64).to_be_bytes();
    Ok(sha256_concat(&[&id, &sh, &ch, &gh, &ts]))
}

/// §9.6 — TxID = H(sender || receiver || amount || nonce || timestamp)
pub fn construct_tx_id(sender: &str, receiver: &str, amount: u64,
                       nonce_hex: &str, timestamp: i64) -> Result<String, L3rsError> {
    let mut amount_buf = [0u8; 32];
    amount_buf[24..].copy_from_slice(&amount.to_be_bytes());
    let nonce = hex::decode(nonce_hex).map_err(|e| L3rsError::Crypto(e.to_string()))?;
    let ts    = (timestamp as u64).to_be_bytes();
    Ok(sha256_concat(&[sender.as_bytes(), receiver.as_bytes(), &amount_buf, &nonce, &ts]))
}

/// §5.10 — Override_Record = H(OID || AUTH || ACTION || TS)
pub fn construct_override_hash(override_id: &str, authority: &str, action: &str, ts: i64) -> String {
    let ts_bytes = (ts as u64).to_be_bytes();
    sha256_concat(&[override_id.as_bytes(), authority.as_bytes(), action.as_bytes(), &ts_bytes])
}
